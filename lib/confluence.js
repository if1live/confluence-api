 ...
 * @param {Function} callback
 */
Confluence.prototype.getSpaceContents = function(space, start, limit, callback){
    var config = this.config;
    request
        .get(`${config.baseUrl}${config.apiPath}/space/${space}/content${config.extension}?start=${start}&limit=${limit}`)
        .auth(config.username, config.password)
        .end(function(err, res){
            processCallback(callback, err, res);
        });
